...
def ridsp_create_test(test_id: str) -> Tuple[str, int]:
    ...
    try:
        ...
        # )
        record = database.TestRecord(
            version=req_body.isa_id, flights=req_body.requested_flights
        )
...
@webapp.route("/ridsp/injection/tests/area", methods=["POST"])
def ridsp_return_area_time() -> Tuple[str, int]:
    try:
        json = flask.request.json
        if json is None:
            raise ValueError("Request did not contain a JSON payload")
        ...
        area_vertices=geo.get_latlngrect_vertices(rect)
        start_time=StringBasedDateTime(t0)
        end_time=StringBasedDateTime(t1)
